...
def preload_windows():
    ...
    try:
        ...
        print("Failed to parse JSON for preloading")
        return
...
def main():
    # initially load all windwos data to track their intial positions, otherwise we won't know how to restore them on the very first pass
    preload_windows()
        ...
        for change in changes:
            ...
            if is_fullscreen(window_size):
                ...
                if window_id in fullscreen_windows:
                    ...
                    # first move the window one column to the left
                    subprocess.run(
                    ...
                    output = window_props.stdout

                    data = json.loads(output)
                    row = data["layout"]["pos_in_scrolling_layout"][1]

                    # move column to the corrrect row
